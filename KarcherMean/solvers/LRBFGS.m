...
params = set_default_para(params, 'StopCriterion', 3, 'int', 0, 6);
...
lntime = 0;
% err_disp = err
% params_error_disp = params.error
% times_disp = times
% params_max_t_disp = params.max_t

% toc;
% b1 = err > params.error
% toc;
% b2 = times < params.max_t
% toc;
...
%     toc;
    ...
    elseif(params.StopCriterion == 4)
        err = err_v;
    elseif(params.StopCriterion == 5)
        err = norm(X{times + 1}.U-X{times}.U,'fro')/norm(X{times}.U,'fro');
    elseif(params.StopCriterion == 6)
        err = dist_mat(X{times + 1}.U,X{times}.U)/norm(X{times}.U,'fro');
    end
